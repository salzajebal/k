...
- [Visitor tracking without inflated counts](visitor-tracking.md) — localStorage visitorId + sessionStorage sessionId + DB unique(session_id) dedup for accurate visitor stats.
- [Production domain TLS scope](production-domain-tls.md) — issue TLS for the apex domain only until www has a valid DNS record, preventing repeated ACME failures.
